<template>
	<div class="rank-list">
		...
		<div class="rank-banner">
			<img :src="imgSrc" alt="">
			<div class="updata">{{updateTime}}</div>
		</div>
		<div class="rank-list-cont">
			<ul>
				<li v-for="(item,index) in songList" @click="play(item.hash,index,item.title)">
					<span v-bind:class="[index == 0 ? 'one' : 'rank-index',index == 1 ? 'two' : '',index == 2 ? 'three' : '']">{{index+1}}</span> {{item.title}}
				</li>
			</ul>
		</div>
	</div>
